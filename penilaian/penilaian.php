<?php
include "koneksi.php";
$slq="select `db_spk`.`tbl_siswa`.`id_siswa` AS `id_siswa`,`db_spk`.`tbl_siswa`.`nama` AS `nama`,`db_spk`.`tbl_nilai`.`c1` AS `c1`,`db_spk`.`tbl_nilai`.`c2` AS `c2`,`db_spk`.`tbl_nilai`.`c3` AS `c3`,`db_spk`.`tbl_nilai`.`c4` AS `c4`,`db_spk`.`tbl_nilai`.`id_nilai` AS `id_nilai` from (`db_spk`.`tbl_siswa` left join `db_spk`.`tbl_nilai` on((`db_spk`.`tbl_siswa`.`id_siswa` = `db_spk`.`tbl_nilai`.`id_siswa`)))";
 ...
 <table class="table table-bordered my-4">
  ...
  <tbody>
   ...
   <tr>
    ...
    <td>
     <input type="hidden" value="<?php echo $row['id_siswa'] ?>" name="id_siswa<?php echo $i;?>">
...
alfa 1 lebih kali dengan nilai 1)</p>
<p>4. Ekstrakulikuler ( <b>Sangat baik (A)</b> nilai : 3; <b>Baik (B)</b> nilai : 2;
 <b>Cukup (C)</b> nilai : 1</p>
</div>
